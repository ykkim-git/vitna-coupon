...
- QR 코드로 접속 → 쿠폰 리스트
- 모든 쿠폰은 **1회용**. `사용하기` → **"정말 사용할까요?"** 확인 후 사용 처리
- 한 번 사용하면 **되돌릴 수 없음**
...
> ⚠️ `id`는 절대 바꾸지 마세요. 사용 여부가 `id`로 저장되기 때문에, id를 바꾸면
...
RLS는 읽기/쓰기 허용으로 두었습니다. (삭제 권한은 주지 않습니다)

### 동기화가 동작하는 방식

- 사용 기록의 기본키를 `<book_id>:<coupon_id>`로 만듭니다 → 두 폰이 동시에 눌러도
  같은 행으로 합쳐집니다. 병합 충돌도, 중복 사용도 생기지 않습니다.
